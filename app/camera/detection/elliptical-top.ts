...
/** Two or three collinear pips cannot establish a plane from their centers.
 * Matching ellipses supply an independent depth estimate for a complete top.
 */
export function readEllipticalTop(pips: readonly EllipticalPip[], width: number, height: number, maxPipArea: number): 2 | 3 | null {
  if ((pips.length !== 2 && pips.length !== 3) || !Number.isFinite(width)
