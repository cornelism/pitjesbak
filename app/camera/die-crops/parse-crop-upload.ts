...
import { MAX_CROP_DIMENSION, MAX_DICE_PER_CAPTURE } from "./limits";
...
export function parseCropUpload(value: unknown) {
  ...
  if (!Array.isArray(batch.crops) || batch.crops.length < 1 || batch.crops.length > MAX_DICE_PER_CAPTURE) throw new Error("Invalid crop count");
  return {
    capturedAt: batch.capturedAt, sourceSize, overviewSize,
    zoom: number(batch.zoom, 1, 20), cameraTilt: number(batch.cameraTilt, 0, MAX_CAMERA_ANGLE),
    crops: batch.crops.map((value, index) => {
      ...
      if (source.width > MAX_CROP_DIMENSION || source.height > MAX_CROP_DIMENSION || !Object.values(source).every(Number.isInteger)) throw new Error("Invalid crop dimensions");
...
export type ParsedCropUpload = ReturnType<typeof parseCropUpload>;
