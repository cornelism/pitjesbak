...
import { useDiceReader, type DiceReaderOptions } from "../use-dice-reader";
...
interface DiceReaderProps extends DiceReaderOptions {
  showPlayArea?: boolean;
}

export default function DiceReader({ showPlayArea = false, ...options }: DiceReaderProps) {
  const {
    overlayRef,
    expectedCount,
    cameraTilt,
    ...
    changeCameraTilt,
    changeExpectedCount,
  } = useDiceReader(options);

  return (
    <div className="pointer-events-none absolute inset-0">
      {showPlayArea && playArea && <PlayAreaOverlay area={playArea} zoom={options.zoom ?? 1} />}
